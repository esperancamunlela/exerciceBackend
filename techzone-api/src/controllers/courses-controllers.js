...
export const getAllCursos = (req, res) => {
  res.status(200).json({ message: "ok", cursos: coursesData });
};

export const getCurso = (req, res) => {
  const id = req.params.id;
  const existCurso = coursesData.find(curso => String(curso.id) === String(id));
...
export const createCourse = (req, res) => {
  const { title, description, durationInHours, level, language, isFree } = req.body;
...
export const deleteCourse = (req, res) => {
  const id = req.params.id;
  const deleteCurso = coursesData.findIndex(curso => String(curso.id) === String(id));
